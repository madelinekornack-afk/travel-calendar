    ...
    for (let d = 0; d < 7; d++) {
      ...
      slots.forEach(trip => {
        ...
        if (isFirst && !isSingleDay && cellsRemaining > 1) {
          ...
          badgesHtml += `<div style="background:${color};height:20px;margin-top:3px;margin-left:-6px;margin-right:${isLast ? '0' : '-6px'};border-radius:0 ${isLast ? '4px 4px' : '0 0'} 0;position:relative;z-index:3;">&nbsp;</div>`;
        } else {
          // Single day trip
