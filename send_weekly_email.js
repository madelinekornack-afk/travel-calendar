    ...
    for (let d = 0; d < 7; d++) {
      ...
      slots.forEach(trip => {
        ...
        if (isFirst && !isSingleDay && cellsRemaining > 1) {
          ...
          badgesHtml += `<div style="background:${color};color:#fff;font-size:10px;font-weight:600;padding:0 5px;border-radius:${rLeft} 4px 4px ${rLeft};margin-top:3px;margin-right:-6px;white-space:nowrap;overflow:visible;width:${widthPct}%;height:20px;line-height:20px;position:relative;z-index:2;">
            ${label}
          </div>`;
        } else if (!isFirst && !isSingleDay) {
          // Continuation day: colored bar edge-to-edge, same height
          badgesHtml += `<div style="background:${color};height:20px;margin-top:3px;margin-left:-6px;margin-right:${isLast ? '0' : '-6px'};border-radius:0 ${isLast ? '4px 4px' : '0 0'} 0;">&nbsp;</div>`;
        } else {
          // Single day trip
